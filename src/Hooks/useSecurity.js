...
export const setAuthData = ({ token, user }) => {
  ...
  // Using a fixed key instead of a unique one:
  const fixedKey = "authData";
  localStorage.setItem("storageKey", fixedKey);
  localStorage.setItem(fixedKey, JSON.stringify({ token, user }));
};
 ...
 */
export const getAuthData = () => {
  const randomKey = localStorage.getItem("storageKey"); // Retrieve the random key (now fixed)
  if (!randomKey) return null; // If no key, return null
